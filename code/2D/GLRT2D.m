...
for i = 1:(length(x)-nbhdSize + 1)
    for j = 1:(length(y)-nbhdSize + 1)
        ...
        numerator = abs(det(1/(N*Jp) * ...
                        sum(f_norm(indX, indY,~isChanged).^2,'all')))^(Jp/2) * ...
                    abs(det(1/(N*(J-Jp)) * ...
                        sum(f_norm(indX, indY, isChanged).^2,'all')))^((J-Jp)/2);
        denominator = abs(det(1/(N*J) * ...
                          (sum(f_norm(indX,indY,:).^2,'all'))))^(J/2);
